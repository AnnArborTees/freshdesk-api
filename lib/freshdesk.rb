...
#   ConnectionError     if there is connection problem with the server
  ...
  %i(tickets ticket_fields ticket_notes users forums solutions companies time_sheets).each do |a|
    fd_define_get a
    fd_define_post a
    fd_define_delete a
    fd_define_put a
  ...
  %i(user_ticket customer_by_email).each do |resource|
...
#   customer_by_email => /contacts.xml
  def mapping(method_name, id = nil)
    case method_name
      ...
      when 'customer_by_email' then File.join(@base_url + 'contacts.xml')
    end
  end

# match with the root name of xml document that freshdesk uses
  def doc_name(name)
    case name
